function [h,T,perm] = polardendrogram_HM(Z,labels,varargin)
...
%Relabel leaves for all 
% for i=minx+1:maxx-1
%     [x,y]=pol2cart((((i-minx)/xrange)*(pi*11/6))+(pi*1/12),1.1);
%     bob = text(x,y,leafLabels{i},'HorizontalAlignment','left');
%     set(bob,'Rotation',7+6*i)
% end
% For spleen only
for i=minx+1:maxx-1
    [x,y]=pol2cart((((i-minx)/xrange)*(pi*11/6))+(pi*1/12),1.1);
    bob = text(x,y,leafLabels{i},'HorizontalAlignment','left');
    set(bob,'Rotation',3+16*i)
end
